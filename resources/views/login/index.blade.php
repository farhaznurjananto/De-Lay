              ...
              <form action="/login" method="post">
                ...
                <ul class="list-unstyled">
                  ...
                  <div class="input-group mb-3">
                    ...
                    <div class="form-floating">
                      <input type="password" class="form-control @error('password') is-invalid @enderror" id="password" name="password" placeholder="Username" required />
                    ...
                    <select class="form-select" aria-label="Default select example" name="actor_id">
                        @foreach ($actors as $actor)
                            @if (old('actor_id') == $actor->id)
                                <option value="{{ $actor->id }}" selected="selected"{{ $actor->name }}>{{ $actor->name }}</option>
                            @else
                ...
                <p class="text-center mt-2">Belum punya akun? <a href="" class="text-decoration-none">Daftar</a></p>
              </form>
            </div>
          </div>
        </div>
